...
import {IMechaoDomainFactory} from "./interface/IMechaoDomainFactory.sol";
import {IMechaoDomain} from "./interface/IMechaoDomain.sol";
import {IMechaoDomainHub} from "./interface/IMechaoDomainHub.sol";
import "./lib/strings.sol";
import "@openzeppelin/contracts/utils/Strings.sol";
import "@openzeppelin/contracts/token/ERC721/extensions/IERC721Metadata.sol";

contract MechaoDomainResolver is Initializable, OwnableUpgradeable {
  ...
  function getDefaultDomain(address _addr, string calldata _tld) public view returns(string memory) {
    ...
    for (uint256 i = 0; i < fLength;) {
      address tldAddr = IMechaoDomainFactory(factories[i]).tldNamesAddresses(_tld);

      if (tldAddr != address(0) && !isTldDeprecated[tldAddr]) {
        return string(IMechaoDomain(tldAddr).defaultNames(_addr));
      }
  ...
  function getDefaultDomains(address _addr) public view returns(string memory) {
    ...
    uint256 fLength = factories.length;
    for (uint256 i = 0; i < fLength;) {
      string[] memory tldNames = IMechaoDomainFactory(factories[i]).getTldsArray();

      for (uint256 j = 0; j < tldNames.length; ++j) {
        string memory tldName = tldNames[j];
        address tldAddr = IMechaoDomainFactory(factories[i]).tldNamesAddresses(tldName);
        string memory defaultName = IMechaoDomain(tldAddr).defaultNames(_addr);
  ...
  function getDomainHolder(string calldata _domainName, string calldata _tld) public view returns(address) {
    ...
    for (uint256 i = 0; i < fLength;) {
      ...
      if (tldAddr != address(0) && !isTldDeprecated[tldAddr]) {
        return address(IMechaoDomain(tldAddr).getDomainHolder(_domainName));
      }
  ...
  function getDomainData(string calldata _domainName, string calldata _tld) public view returns(string memory) {
    ...
    for (uint256 i = 0; i < fLength;) {
      ...
      if (tldAddr != address(0) && !isTldDeprecated[tldAddr]) {
        return string(IMechaoDomain(tldAddr).getDomainData(_domainName));
      }
  ...
  function getDomainTokenUri(string calldata _domainName, string calldata _tld) public view returns(string memory) {
    ...
    for (uint256 i = 0; i < fLength;) {
      ...
      if (tldAddr != address(0) && !isTldDeprecated[tldAddr]) {
        (, uint256 _tokenId, , ) = IMechaoDomain(tldAddr).domains(_domainName);
  ...
  function getTldAddress(string calldata _tldName) public view returns(address) {
    uint256 fLength = factories.length;
    for (uint256 i = 0; i < fLength;) {
      address tldAddr = IMechaoDomainFactory(factories[i]).tldNamesAddresses(_tldName);
  ...
  function _isHubAdmain() internal {
        require(IMechaoDomainHub(hubAddress).checkHubAdmin(_msgSender()), "Not Hub Admin");
    }
}